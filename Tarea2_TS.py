...
for h in range(nh):     # Ciclo para cada hora
    ...
    fe = SF[pl_nots,:][:,pos_g] @ p_gt[:,h] - SF[pl_nots,:]@dda_bus
    fv = (SF[pl_nots,:] @ A[pl_ts,:].T) @ f[nl_ts,h]
    m.addConstr(fe+fv <= FM_lnots, name = 'fe_p')
    m.addConstr(fe+fv >= -FM_lnots, name = 'fe_n')

    #Restricciones sistema de transmisión lineas candidatas
    f1 = SF[pl_ts,:][:,pos_g] @ p_gt[:,h] - SF[pl_ts,:]@dda_bus
    f2 = f[:,h] - (SF[pl_ts,:]@A[pl_ts,:].T)@f[:,h] 

    m.addConstr(f1-f2 <= np.diag(FM_lts) @ s_ts[:,h], name = 'fs1_p') # 1
    m.addConstr(f1-f2 >= -np.diag(FM_lts) @ s_ts[:,h], name = 'fs1_n')
...
#Rampas 
    ...
    if h==0:
        ...
        m.addConstr( -pbar_gt[:,h] >= - (P_g0 + np.np.diag(RUg) @ b_g0 + np.np.diag(SUg)@(b_gt[:,h]-b_g0) + Pmax - np.np.diag(Pmax)@b_gt[:,h] )/Sb, name="CA_eq11" )
        # (MW en hora 0 - MW en hora 1) <= (Rampa Down * Gen ON en hora 1)  +  (Shut Down * (Gen ON en hora 0 - hora 1)) + (Pmax) - (Pmax * Gen On en hora 0)
        m.addConstr( -( P_g0/Sb - p_gt[:,h] ) >= -( np.np.diag(RDg)@b_gt[:,h] + np.np.diag(SDg)@(b_g0-b_gt[:,h]) + Pmax - np.np.diag(Pmax)@b_g0 )/Sb, name="CA_eq13" )

    else:
        # (P_bar en hora h) <= (MW en hora h-1) + (Rampa Up * Gen ON en hora h-1)  +  (Start Up * (Gen ON en hora h - hora h-1)) + (Pmax) - (Pmax * Gen On en hora h)
        m.addConstr( -pbar_gt[:,h] >= - p_gt[:,h-1] - ( np.np.diag(RUg) @ b_gt[:,h-1] + np.np.diag(SUg)@(b_gt[:,h]-b_gt[:,h-1]) + Pmax - np.np.diag(Pmax)@b_gt[:,h] )/Sb, name="CA_eq11" )
        # (MW en hora h-1 - MW en hora h) <= (Rampa Down * Gen ON en hora h)  +  (Shut Down * (Gen ON en hora h-1 - hora h)) + (Pmax) - (Pmax * Gen On en hora h-1)
        m.addConstr( -( p_gt[:,h-1] - p_gt[:,h] ) >= -( np.np.diag(RDg)@b_gt[:,h] + np.np.diag(SDg)@(b_gt[:,h-1]-b_gt[:,h]) + Pmax - np.np.diag(Pmax)@b_gt[:,h-1] )/Sb, name="CA_eq13" )

for h in range(nh-1):
    # (P_bar en hora h) <= (Pmax * Gen On en hora h+1) + (Shut Down * (Gen On en hora h - hora h+1)
    m.addConstr( -pbar_gt[:,h] >= -( np.np.diag(Pmax)@b_gt[:,h+1] + np.np.diag(SDg)@( b_gt[:,h]- b_gt[:,h+1] ) )/Sb, name="CA_eq12" ) 
